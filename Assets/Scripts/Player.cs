...
public class Player : MonoBehaviour, IkitchenObjectParent
{
   ...
   private void HandleMovement()
   {
      ...
      {   // Attemting to move in x direction only
         ...
         canMove = movDir.x != 0 && !Physics.CapsuleCast(transform.position, transform.position + Vector3.up * playerHeight, playerRaduis, moveDirx, moveDistance);
         if (canMove)
         {
         ...
         { // Attempting to move in z direction only
            ...
            canMove = movDir.z != 0 && !Physics.CapsuleCast(transform.position, transform.position + Vector3.up * playerHeight, playerRaduis, moveDirz, moveDistance);
            if (canMove)
